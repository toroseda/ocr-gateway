...
import ae.etisalat.eim.ocr.gateway.domain.EntityAuditEvent;
...
public interface EntityAuditEventRepository extends JpaRepository<EntityAuditEvent, Long> {
    ...
    @Query("SELECT bae FROM EntityAuditEvent bae where bae.entityType = :type and bae.entityId = :entityId and " +
        "bae.commitVersion =(SELECT max(a.commitVersion) FROM EntityAuditEvent a where a.entityType = :type and " +
